...
public class ConvertToFJTaskEditorAction implements IEditorActionDelegate{
	...
	public void run(IAction action) {
		...
		MessageDialog.openError(getShell(), "Error for ConvertToConcurrentHashMap", "ConvertToConcurrentHashMap not applicable for current selection");  //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	private boolean isTypeSignatureForParametrized(String typeSignature) {
		...
		return ((typeSignature.indexOf("QHashMap<") != -1) || (typeSignature.indexOf("QMap<") != -1)); 	 //$NON-NLS-1$ //$NON-NLS-2$
	}
